  ...
  <div class="create-post">
    ...
    <div class="container">
      <div :class="{ invisible: !error }" class="err-message">
        <p><span>Hata : </span>{{ this.errorMsg }}</p>
      </div>
      <div class="blog-info">
        <input
          type="text"
        ...
        <div class="upload-file">
          <label for="blog-photo">Kapak Fotoğrafını Yükle</label>
          <input
            type="file"
            ref="blogPhoto"
            id="blog-photo"
      ...
      <div class="blog-actions">
          <button @click="updateBlog" :to="{ name: 'ViewBlog' }">
            Değişiklikleri Kaydet
          </button>
        <router-link :to="{ name: 'BlogPreview' }" class="router-button"
          >Değişiklikleri Önizle</router-link
        >
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    async updateBlog() {
      ...
      if (this.blogTitle.length !== 0 && this.blogHTML.length !== 0) {
        ...
        await this.$store.dispatch("updatePost", this.routeID);
        this.loading = false;
        this.$route.push({ name: "ViewBlog", params: { blogid: dataBase.id } });
        return;
      }
      this.error = true;
